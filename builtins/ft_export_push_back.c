...
static char *export_operator_equal_or_plus(char *export_envp_content, char *splited_export)
{
    int switch_case;
    if (export_operator(splited_export) == 1)
    {
        ...
        switch_case = 3;
    return (switch_case_export(export_envp_content , splited_export , switch_case));
}
...
void push_back(t_list **export_envp, char *splited_export)
{
    ...
    if (key_is_already_exist(*export_envp, key))
    {
        ...
        export_envp_content = export_operator_equal_or_plus(export_envp_content, splited_export);
